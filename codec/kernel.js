...
// optional overlays (see overlay.js). An overlay is { graph, install?, installChecks? }:
//   graph                  its JSON-LD (chains, structs, rules)
//   install(codec)         runs before the chain is bound: proof-of-work hashes, derived fields
//   installChecks(engines) runs after the engines exist: the checks behind its rules
// With no overlays this is exactly the hand wiring the tests and apps have always done.
