...
| 5.2 | Script Execution | ✅ | `codec/interpreter.js`, gated by `scripts` rule | full opcode set incl. `OP_CHECKLOCKTIMEVERIFY` / `OP_CHECKSEQUENCEVERIFY`; legacy + BIP143 + BIP341 sighash, Knots unified sighash from the overlay's `unifiedSighashParam` height (166 Knots vectors + real txbt4 spends); **differentially tested against Core's `script_tests.json`** (1,191 cases — bare, P2SH redeem, witness programs) |
